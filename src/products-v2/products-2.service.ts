...
import { PRODUCT_DATA } from './product-v2.data';
...
export class ProductsV2Service {
  ...
  private addProperty(products) {
    return products.map(
      product => ({ ...product, tags: this.generateTags(product) })
    );
  }

  private generateTags(product: Product): string[] {
    const availableTags =
      ['#new', '#old', '#limited_offer',
        '#last_items', '#nofilter', '#spartacus',
        '#divante', '#training', '#best', '#angular'];
